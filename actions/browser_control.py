    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using automation profile")
                nexus_automation = str(Path.home() / ".nexus_profiles" / "firefox_nexus")
                Path(nexus_automation).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(nexus_automation, **kwargs)
        ...
        # automation). Fall back to a persistent automation profile —
        # accounts logged in here once stay logged in on later sessions too.
        nexus_profile = str(Path.home() / ".nexus_profiles" / self.browser_name)
        Path(nexus_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with automation profile: {nexus_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(nexus_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with automation profile "
