...
# (screen_process, close_camera, save_memory, manage_monitor, shutdown_nexus,
# system_status).
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are NEXUS, an intelligent personal AI assistant designed to help the user "
            "interact with their computer, write and execute code, manage files, automate "
            "tasks, understand visual information, and perform multi-step actions safely. "
...
TOOL_DECLARATIONS = [
    ...
    # core.action_loader (see NexusLive.__init__)
    {
        "name": "system_status",
        "description": (
    ...
    },
    {
        ...
        "description": (
            ...
            "NEXUS checks these topics once a day and alerts the user when there is a new development. "
    ...
    },
    {
        "name": "shutdown_nexus",
        "description": (
            "Shuts down NEXUS completely. "
            "Call this when the user expresses intent to end the session, "
            "close the assistant, says goodbye, or stops NEXUS. "
...
class NexusLive:
    ...
    def sleep(self, reason: str = "timeout") -> None:
        ...
        self.ui.write_log(f"SYS: Sleeping — {reason}. Say the wake phrase to wake me.")
    ...
    def plugin_say(self, instruction: str) -> None:
        """
        Thread-safe speech channel for plugins: lets a plugin ask NEXUS to
...
# Respect wake-word sleep: a typed command must not be answered while
        ...
        if self._wake_enabled and not self._awake:
            self.ui.write_log("SYS: I'm asleep — say the wake phrase or tap WAKE NOW first.")
            return
    ...
    def interrupt(self) -> None:
        """Stop NEXUS mid-speech: drain queued audio and open mic immediately."""
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "NEXUS").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "NEXUS"
            _user_name = ""
        ...
        cfg = dict(
            ...
            # window — NEXUS can stay in one conversation for hours
        ...
        if self._enhanced_live:
            # Proactive audio: NEXUS stays silent when speech isn't addressed
    ...
    async def _listen_audio(self):
        ...
        def callback(indata, frames, time_info, status):
            ...
            # streamed, so NEXUS can't respond to speech not addressed to it and
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if self._pending_vision and self.session:
                                ...
                                # Set next turn_complete behaviour depending on angle
                                if self._vision_cam_active:
                                    # Camera: keep busy until NEXUS finishes speaking the answer
    ...
    async def _play_audio(self):
        ...
        try:
            while True:
                ...
                # Drive the HUD waveform from NEXUS's own voice while speaking.
                try:
    ...
    async def _run_background_monitor(self) -> None:
        ...
        while True:
            if self.session and self._awake:
                # Don't interrupt if user spoke recently or NEXUS is mid-sentence
                with self._speaking_lock:
                    speaking = self._is_speaking
        ...
        while True:
            try:
                ...
                if self.session:
                    ...
                    # has no desktop WAKE button — so it wakes NEXUS if asleep.
                    if self._wake_enabled and not self._awake:
            ...
            try:
                ...
                async with (
                    ...
                    if self._wake_enabled:
                        ...
                        self._awake = False
                        self.ui.set_state("SLEEPING")
                        self.ui.write_log("SYS: NEXUS online — sleeping. Say the wake phrase to wake me.")
                    else:
                        self._awake = True
                        self.ui.set_state("LISTENING")
