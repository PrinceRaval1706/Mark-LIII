"""
dashboard/server.py — NEXUS Local HTTP Dashboard
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'NEXUS-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"NEXUS Dashboard Port {port}"
        prog_rule  = "NEXUS Dashboard Python"
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until NEXUS is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        nexus_key = (certs / "nexus.key").exists()
        nexus_crt = (certs / "nexus.crt").exists()
        jarvis_key = (certs / "jarvis.key").exists()
        jarvis_crt = (certs / "jarvis.crt").exists()
        if nexus_key and nexus_crt:
            return True
        if jarvis_key and jarvis_crt:
            print("[Dashboard] Using legacy 'jarvis' cert files — consider renaming to 'nexus'")
            return True
        return False

    @staticmethod
    def _ssl_paths() -> tuple[Path, Path]:
        """Return (key_path, cert_path). Prefers nexus cert files, falls back to jarvis."""
        certs = BASE_DIR / "config" / "certs"
        nexus_key = certs / "nexus.key"
        nexus_crt = certs / "nexus.crt"
        if nexus_key.exists() and nexus_crt.exists():
            return nexus_key, nexus_crt
        jarvis_key = certs / "jarvis.key"
        jarvis_crt = certs / "jarvis.crt"
        if jarvis_key.exists() and jarvis_crt.exists():
            return jarvis_key, jarvis_crt
        return nexus_key, nexus_crt
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in NEXUS to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('nexus_token','{tok}');
  sessionStorage.setItem('nexus_key','{key}');
  localStorage.setItem('nexus_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to NEXUS…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key, ssl_cert = self._ssl_paths()
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in NEXUS UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
